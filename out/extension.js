...
function insertSnippet(before, after, space, replaceComma) {
    const editor = vscode.window.activeTextEditor;
...
function activate(context) {
    context.subscriptions.push(vscode.commands.registerCommand("flutter-widget-wrap.wrapInSingleChildWidget", () => insertSnippet("${1:widget}(\n" + getSpacer() + "child: $2", "\n)", getSpacer())));
    context.subscriptions.push(vscode.commands.registerCommand("flutter-widget-wrap.wrapInBuilderWidget", () => insertSnippet("${1:builderWidget}(\n" +
        getSpacer() +
        "${2:builder}: (${3:context}, ${4:constructor}) {\n" +
        getSpacer() +
        getSpacer() +
        "return ", "\n},\n" + ")", getSpacer(), true)));
    context.subscriptions.push(vscode.commands.registerCommand("flutter-widget-wrap.wrapInChildrenWidget", () => insertSnippet("${1:widget}(\n" +
        getSpacer() +
        "children: [\n" +
